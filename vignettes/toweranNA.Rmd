...
This is a followed by a methodological explanation of how the package
works, then a followup to the vocabulary example, and a second example
in the time series realm.
...
Internallly, the function **regtools::TStoX()** transforms the data
...
Could a **predict()** method be added to packages like
**Amelia** and **mice**?
...
Thus **Amelia**, **mice** etc. could be ``Tower-ized''. They
would not have the central focus on Y that Tower has, but reasonable
**predict()** methods could be developed for them via
Tower-ization.
...
Random forests packages such as **randomForest** and **grf**
